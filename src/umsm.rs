...
/// Simpler pippenger for u64 scalars.
pub fn u64msm<G: CurveGroup>(bases: &[G::Affine], scalars: &[u64]) -> G {
    let mut buckets = [G::zero(); 64];
    for (base, scalar) in bases.iter().zip(scalars.iter()) {
        buckets.iter_mut().enumerate().for_each(|(i, bucket)| {
            if scalar & (1u64 << i) != 0 {
                bucket.add_assign(base);
...
fn test_u64msm() {
    ...
    let bases = (0..64)
        .map(|_| G::rand(&mut rng).into_affine())
        .collect::<Vec<_>>();
    let scalars = (0..64).map(|_| rng.gen()).collect::<Vec<u64>>();
    let ff_scalars = scalars.iter().map(|&x| F::from(x)).collect::<Vec<_>>();

    let expected = G::msm(&bases, &ff_scalars).unwrap();
    let actual: G = u64msm(&bases, &scalars);
    assert_eq!(expected, actual)
}
